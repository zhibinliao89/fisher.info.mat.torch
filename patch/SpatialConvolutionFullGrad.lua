local SpatialConvolutionFullGrad, parent =
    torch.class('cudnn.SpatialConvolutionFullGrad', 'nn.SpatialConvolution')
local ffi = require 'ffi'
local find = require 'cudnn.find'
local errcheck = cudnn.errcheck
local checkedCall = find.checkedCall

function SpatialConvolutionFullGrad:__init(nInputPlane, nOutputPlane,
...
function SpatialConvolutionFullGrad:resetWeightDescriptors(desc)
    -- for compatibility
...
function SpatialConvolutionFullGrad:fastest(mode)
...
function SpatialConvolutionFullGrad:setMode(fmode, bdmode, bwmode)
    if fmode ~= nil then
        self.fmode = fmode
    end
    if bdmode ~= nil then
...
function SpatialConvolutionFullGrad:resetMode()
    self.fmode = nil
    self.bdmode = nil
    self.bwmode = nil
    return self
...
function SpatialConvolutionFullGrad:noBias()
   self.bias = nil
   self.gradBias = nil
   self.gradBiasFull = nil
   return self
...
function SpatialConvolutionFullGrad:checkInputChanged(input)
...
function SpatialConvolutionFullGrad:createIODescriptors(input)
   local batch = true
   if input:dim() == 3 then
      input = input:view(1, input:size(1), input:size(2), input:size(3))
      batch = false
   end
   if SpatialConvolutionFullGrad.checkInputChanged(self, input) then
...
function SpatialConvolutionFullGrad:updateOutput(input)
...
function SpatialConvolutionFullGrad:updateGradInput(input, gradOutput)
...
function SpatialConvolutionFullGrad:gradFullSpacing(input)
    if self.getFullGrad then
        ...
        if self.gradWeightFull == nil then
            self.gradWeightFull = torch.Tensor(input:size(1), self.weight:size(1), self.weight:size(2), self.weight:size(3), self.weight:size(4)):cuda():zero()
...
function SpatialConvolutionFullGrad:accGradParameters(input, gradOutput, scale)
    ...
    if self.getFullGrad then
        self:createIODescriptors(input:narrow(1,1,1))
        local finder = find.get()
        ...
        local cIndex
        local input_Offset_I = input:size(2)*input:size(3)*input:size(4)
        local gradOutput_Offset_I = gradOutput:size(2)*gradOutput:size(3)*gradOutput:size(4)
        local gradWeight_Offset_I = self.gradWeight:nElement()
        if self.bias then
            local gradBias_Offset_I = self.gradBias:nElement()
        end
        for luaIndex = 1, input:size(1) do
            ...
            if self.bias then
                errcheck('cudnnConvolutionBackwardBias', cudnn.getHandle(),
                         self.scaleT:data(),
                         self.oDescForBias[0], gradOutput:data() + cIndex*gradOutput_Offset_I,
                         cudnn.scalar(input, 0), --beta:ignore what's in the dest
                         self.biasDesc[0], self.gradBiasFull:data() + cIndex*gradBias_Offset_I)
            end

            local extraBuffer, extraBufferSize = cudnn.getSharedWorkspace()
            for g = 0, self.groups - 1 do
                -- gradWeight
                checkedCall(self,'cudnnConvolutionBackwardFilter', cudnn.getHandle(), --handle
                         self.scaleT:data(), --alpha
                         self.iDesc[0], input:data() + cIndex*input_Offset_I + g*self.input_offset, --srcDesc *srcData
                         self.oDesc[0], gradOutput:data() + cIndex*gradOutput_Offset_I + g*self.output_offset, --diffDesc, *diffData
                         self.convDesc[0], --convDesc, 
                         -- self.bwdFilterAlgType[0], --algo
                         -- self.extraBuffer:data(), self.extraBufferSizeInBytes, --*workSpace, workSpaceSizeInBytes
                         bwdFilterAlgo,
                         extraBuffer, extraBufferSize,
                         cudnn.scalar(input, 0), --beta:ignore what's in the dest
                         self.weightDesc[0], self.gradWeightFull:data() + cIndex*gradWeight_Offset_I + g*self.weight_offset); --gradDesc, *gradData
            end
        end
        if self.bias then
            self.gradBias:add(torch.sum(self.gradBiasFull, 1):squeeze())
        end
        self.gradWeight:add(torch.sum(self.gradWeightFull, 1):squeeze())   
    else
      -- the original code for minibatch calculation
      self:createIODescriptors(input)
      local finder = find.get()
      local bwdFilterAlgo = finder:backwardFilterAlgorithm(self, { self.iDesc[0], self.input_slice, self.oDesc[0],
                                                                 self.output_slice, self.convDesc[0], self.weightDesc[0], self.weight})
...
function SpatialConvolutionFullGrad:clearDesc()
    self.weightDesc = nil
    self.biasDesc = nil
    self.convDesc = nil
    self.iDesc = nil
...
function SpatialConvolutionFullGrad:write(f)
    self:clearDesc()
    local var = {}
...
function SpatialConvolutionFullGrad:clearState()
   self:clearDesc()
   nn.utils.clear(self, '_input', '_gradOutput', 'input_slice', 'output_slice', 'gradWeightFull', 'gradBiasFull')
