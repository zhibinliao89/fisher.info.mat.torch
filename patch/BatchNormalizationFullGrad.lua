...
local BN, parent = torch.class('nn.BatchNormalizationFullGrad', 'nn.Module')
...
function BN:gradFullSpacing(input)
   if self.getFullGrad then
      ...
      if self.gradWeightFull == nil then
         self.gradWeightFull = torch.Tensor(input:size(1), self.weight:size(1)):cuda():zero()
...
local function backward(self, input, gradOutput, scale, gradInput, gradWeight, gradBias)
   ...
   if self.affine then
      ...
      if self.getFullGrad then
         local mean = self.save_mean:view(1,self.save_mean:size(1), 1, 1):expandAs(input)
         local std = self.save_std:view(1,self.save_std:size(1), 1, 1):expandAs(input)
         local x = torch.cmul(input - mean, std)

         self.gradWeightFull:copy(torch.sum(torch.cmul(x, gradOutput):view(torch.LongStorage{input:size(1), input:size(2),-1}),3):squeeze())
         self.gradBiasFull:copy(torch.sum(gradOutput:view(torch.LongStorage{gradOutput:size(1), gradOutput:size(2),-1}),3):squeeze())
      end
   end
